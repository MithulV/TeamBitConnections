    ...
    const result = await db.begin(async (t) => {
      ...
      if (address) {
        [createdAddress] = await t`
          ...
          VALUES (${contactId}, ${address.street || null}, ${address.city || null}, ${address.state || null}, ${address.country || null}, ${address.zipcode || null}) 
          RETURNING *
        `;
      }
    ...
    const result = await db.begin(async (t) => {
      ...
      if (!contact_id) {
        ...
        if (existingContact) {
          ...
          console.log(`Found existing contact: ${existingContact.contact_id}. Updating instead of creating duplicate.`);
      ...
      if (address && activeContactId) {
        [updatedAddress] = await t`
          INSERT INTO contact_address (contact_id, street, city, state, country, zipcode)
          VALUES (${activeContactId}, ${address.street || null}, ${address.city || null}, ${address.state || null}, ${address.country || null}, ${address.zipcode || null})
