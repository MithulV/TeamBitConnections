    ...
    const result = await db.begin(async (t) => {
      ...
      if (events && events.length > 0) {
        for (const event of events) {
          const [newEvent] =
            await t`INSERT INTO event (contact_id, event_name, event_role, event_date, event_held_organization, event_location, verified) VALUES (${contactId}, ${event.event_name}, ${event.event_role}, ${event.event_date}, ${event.event_held_organization}, ${event.event_location}, ${event.verified || false}) RETURNING *`;
...
export const DeleteContact = async (req, res) => {
  ...
  try {
    await db.begin(async (t) => {
      ...
      const [contact] =
        await t`SELECT contact_id FROM contact WHERE contact_id = ${id}`;
      if (!contact) {
