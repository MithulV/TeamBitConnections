...
const RecentEventsTimeline = ({ contacts }) => {
  const recentEvents = useMemo(() => {
    ...
    contacts.slice(0, 15).forEach(contact => {
      if (contact.created_at) {
        events.push({
          ...
          time: parseISO(contact.created_at),
        ...
        {recentEvents.map((event, index) => (
          <div key={event.id} className="flex items-start gap-3 p-3 hover:bg-gray-50 rounded-lg transition-colors">
            <div className="flex-shrink-0 mt-1">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${event.category === 'A' ? 'bg-red-100 text-red-600' :
                event.category === 'B' ? 'bg-orange-100 text-orange-600' :
                  event.category === 'C' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'
                }`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-1">{getTimeAgo(event.time)}</p>
            </div>
          </div>
        ))}
      </div>
